            ...
            state.transactions.when(
              ...
              data: (_) {
                ...
                for (var tx in state.filteredList) {
                  String dateKey = DateFormat('yyyy-MM-dd').format(tx.transactionDate);
                  ...
                  groupWidgets.add(Column(children: [
                    Container(color: Colors.white, child: Column(children: [
                      ...
                      Column(children: txList.map((tx) {
                        ...
                        // 💡 ĐÃ SỬA: Xử lý hiển thị Icon thông minh như trang Danh mục
                        Widget iconWidget;
                        if (category != null && category.iconData != null) {
                          iconWidget = Icon(category.iconData, color: category.color, size: 20);
                        } else if (category != null && category.icon != null && category.icon!.isNotEmpty && category.icon != 'null') {
                          final parsed = int.tryParse(category.icon!);
                          if (parsed != null) {
                            iconWidget = Icon(IconData(parsed, fontFamily: 'MaterialIcons'), color: category.color, size: 20);
                          } else {
                            iconWidget = Text(category.icon!, style: TextStyle(fontSize: 20, color: category.color));
                          }
                        } else {
                          iconWidget = Icon(Icons.category, color: category?.color ?? Colors.grey, size: 20);
                        }
                            ...
                            child: InkWell(
                                ...
                                child: Padding(
                                    ...
                                    child: Row(children: [
                                      Container(
                                          width: 44, height: 44,
                                          decoration: BoxDecoration(color: (category?.color ?? Colors.grey).withOpacity(0.15), shape: BoxShape.circle),
                                          child: Center(child: iconWidget) // <-- Dùng widget icon thông minh
                                      ),
                                      const SizedBox(width: 16),
                                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                        Text(tx.note.isNotEmpty ? tx.note : tx.categoryId, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: Colors.black)),
                                        const SizedBox(height: 4),
                                        Text(tx.categoryId, style: TextStyle(color: Colors.grey.shade500, fontSize: 13))
                                      ])),
                                      Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                                        ...
                                        const SizedBox(height: 4),
                                        Text(DateFormat('HH:mm').format(tx.transactionDate), style: TextStyle(color: Colors.grey.shade400, fontSize: 12))
                                      ])
                                    ])
                                )
                            )
